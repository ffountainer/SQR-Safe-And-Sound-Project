...
def _ensure_existing_machine_metadata(connection) -> None:
    """Populate metadata for already seeded machines
    when the schema was extended."""
    rows = (
        connection.execute(text("SELECT id, floor, building FROM machines"))
        .mappings()
        .all()
    )
    mapping = {
    ...
    for row in rows:
        if row["id"] in mapping and (
                row["floor"] is None or row["building"] is None):
        ...
        connection.execute(
            text(
                ...
                CREATE TABLE IF NOT EXISTS reports (
                    ...
                    (status IN ('free', 'busy', 'unavailable')),
...
def fetch_machine_history(
        machine_id: int, limit: int = 10) -> list[dict[str, Any]]:
    ...
    with engine.connect() as connection:
        result = connection.execute(
            query, {"machine_id": machine_id, "limit": limit})
        return [dict(row._mapping) for row in result]
...
def insert_report(
    ...
    query = text(
        """
        INSERT INTO reports (machine_id, timestamp,
        status, time_remaining, reporter_name)
        VALUES (:machine_id, :timestamp,
        :status, :time_remaining, :reporter_name)
        RETURNING id, machine_id, timestamp,
        status, time_remaining, reporter_name
        """
    )
    params = {
